...
export async function generateSyntheticBenglishData(count: number = 20): Promise<TranslationResult[]> {
  const apiKey = getApiKey();
  if (!apiKey || apiKey === "AI Studio Free Tier") {
    throw new Error("Gemini API key is missing.");
  }

  const systemInstruction = `You are a dataset generator for Benglishify. 
  Generate ${count} unique, realistic conversation pairs in Benglish (Bengali in English letters), Bengali script, and English.
  Focus on common internet slang, social media comments, and casual chat patterns used by Bengalis.
  
  Return the result as a JSON array of objects with these keys:
  - benglish: Bengali written in English letters
  - english: Natural English translation
  - bengali: Bengali script
  
  Make them diverse: greetings, asking for help, expressing emotions, discussing food, sports (cricket), and daily life.`;

  const response = await ai.models.generateContent({
    model: modelName,
    contents: [{ parts: [{ text: `Generate ${count} conversation items.` }] }],
    config: {
      systemInstruction,
      temperature: 0.8,
      responseMimeType: "application/json",
